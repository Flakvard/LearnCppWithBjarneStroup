...
int main()
{
   ...
   * Comments: This one took a while for me to finish.
   * I accomplise this one by reading int inputs into
   * a vector. 
   * Then sorting the vector and check if the current 
   * number is the same as the last one. 
   * if it is then increment count by one each time. 
   * Also store/track the largest double and the largest count.
   * When the next number comes it checks again for doubles and
   * counts the doubles. If it has a higher count than the previous
   * number, then assign it as the largest double and largest cout.
   * Then print out the largest double(mode) and how many times it 
   * was counted.
   */
  double largestval = 0;
  ...
  while(cin>>choose_a_value>>unit){
    ...
    for(int k = 1; k < Chosen_values.size();++k){
      if(Chosen_values.size()>1){
       cout<<"Nr:"<<k<<" "<<Chosen_values[k]
           <<'\t'<<'\n'; 

        duplicates = Chosen_values[k]-Chosen_values[k-1];
        if(duplicates == 0){
          if(Chosen_values[k] != trackDuplicate){
             trackDuplicate = Chosen_values[k];
             countDuplicate = 0; //rest value
             ++countDuplicate;
          }else {
             ++countDuplicate;  
          }
    ...
    cout<<"The mode is: "<<trackLargestDuplicate
        <<" and was counted "<<trackCount<<'\n';
    }
